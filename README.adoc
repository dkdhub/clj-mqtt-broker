...
=== Advanced Broker with programmatic config
...
| `:port-tcp` | `"disabled"` | TCP port for MQTT
| `:port-ws` | `"disabled"` | WebSocket port
| `:host` | `"0.0.0.0"` | Bind address
| `:passwords-path` | `""` | Path to password file
| `:anonymous?` | `false` | Allow anonymous connections
| `:ws-path` | `"/"` | WebSocket path
| `:persistence-type` | none | `:h2` or `:segmented`
| `:data-path` | `"mqtt-data"` | Persistence data directory
| `:message-size` | `8092` | Max MQTT message size in bytes
| `:allow-zero-byte-client-id?` | none | Allow MQTT5 zero-byte client IDs
| `:buffer-flush-ms` | none | Buffer flush interval in milliseconds
| `:netty-native?` | none | Use native transport (kqueue/epoll)
|===
